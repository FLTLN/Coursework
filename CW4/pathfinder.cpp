...
void find_path(ROAD_NODE ** path,PAR * where, int from, int to,int refresh)
{
	...
	if (refresh)
	{
		...
		ROOM * current_room = find_room(where->rooms_courners_list, from);
	...
	else
	{
		...
		if ((*path)->unvisited_U && (*path)->room->UN_R && find_room(where->rooms_courners_list, (*path)->room->UN_N)->not_wisited)
		{
			(*path)->unvisited_U = 0;
			(*path)->room->not_wisited = 0;
			find_path(path, where, (*path)->room->UN_N, to,1);
		}
		else if ((*path)->unvisited_B && (*path)->room->BN_R && find_room(where->rooms_courners_list, (*path)->room->BN_N)->not_wisited)
		{
			(*path)->unvisited_B = 0;
			(*path)->room->not_wisited = 0;
			find_path(path, where, (*path)->room->BN_N, to,1);
		}
		else if ((*path)->unvisited_R && (*path)->room->RN_R && find_room(where->rooms_courners_list, (*path)->room->RN_N)->not_wisited)
		{
			(*path)->unvisited_R = 0;
			(*path)->room->not_wisited = 0;
			find_path(path, where, (*path)->room->RN_N, to,1);
		}
		else if ((*path)->unvisited_L && (*path)->room->LN_R && find_room(where->rooms_courners_list, (*path)->room->LN_N)->not_wisited)
		{
			(*path)->unvisited_L = 0;
			(*path)->room->not_wisited = 0;
		...
		{
			find_room(where->rooms_courners_list, (*path)->room->number)->is_false_road = 1;
